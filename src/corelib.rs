...
const _FILE_PATH: &str = "mod.json";
const INIT_RATING: f32 = 0.5;
const SPAM_PRB_THRES: f32 = 0.8;
...
pub fn score(msg: &str) -> Result<f32, io::Error> {
    let mut file = File::open(_FILE_PATH)?;
...
pub fn identify(msg: &str) -> Result<bool, io::Error> {
    let score = score(msg)?;
    let is_spam = score > SPAM_PRB_THRES;
    Ok(is_spam)
}
